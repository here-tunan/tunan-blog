...
- **多语言内容**: 支持 `/zh-CN/blog/...` 和 `/en/blog/...` 这类语言前缀路由，支持 UI 文案、文章标题、slug、正文内容和搜索结果的多语言展示，并可按每篇文章自己的默认语言进行回退。
...
- **文章全功能管理 (CRUD)**: 使用 Markdown 编辑器，对文章进行创建、读取、更新和删除操作。
- **多语言文章编辑器**: 在后台按文章设置默认语言，并通过语言标签页管理 `zh-CN` / `en` 等翻译版本。
- **服务端分页**: 后台列表页使用分页 API 查询，避免一次性加载全部数据。
...
- `article` 保存文章级元信息，例如类型、默认语言、浏览量、点赞数、删除状态和时间戳。
- `article_translation` 保存语言相关字段，例如标题、slug、Markdown 正文、SEO 字段、发布状态和发布时间。

每篇文章都可以设置自己的默认语言。如果访问者打开了一个不存在的语言版本，前台会回退到该文章自己的默认语言版本。因此，中文优先、英文优先、部分翻译的文章都可以同时存在。

前台 UI 和命令面板搜索都支持多语言。后台提供语言标签页用于编辑翻译版本，并且列表页支持服务端分页，适合数据量增长后的管理场景。
