...
export default function Navigation({ setCommandPaletteOpen }: { setCommandPaletteOpen: (open: boolean) => void }) {
  ...
  const articleItems = [
    {
      ...
      description: dictionary.nav.blogDescription,
    },
    {
      name: dictionary.nav.weekly,
      href: withLocale(locale, '/weekly'),
      description: dictionary.nav.weeklyDescription,
    },
    {
      name: dictionary.nav.translations,
      href: withLocale(locale, '/translations'),
      description: dictionary.nav.translationsDescription,
    },
    {
      name: dictionary.nav.history,
      href: withLocale(locale, '/history'),
      description: dictionary.nav.historyDescription,
    },
  ];
  ...
  const languageHref = (targetLocale: Locale) => {
    const nextPath = replaceLocale(pathname, targetLocale);
  ...
  return (
    <div
      ...
      <nav className="flex items-center gap-1 sm:gap-3">
        ...
        <ArticleDropdown items={articleItems} label={dictionary.nav.articles} />
      </nav>
      <div className="flex items-center gap-1 sm:gap-3">
        ...
        <div className="flex items-center gap-1">
          <Link
            ...
            title={dictionary.nav.friends}
          >
            <img src="/assets/icons/paper-flight.png" alt={dictionary.nav.friends} className="w-4 h-4 opacity-70 group-hover:opacity-100 transition-opacity" />
          </Link>
          ...
          <a
            ...
            title={dictionary.nav.folo}
          >
            <svg width="14" height="14" viewBox="0 0 24 24" className="flex-shrink-0">
              <title>Folo</title>
              <path fill="#ff5c00" d="M5.382 0h13.236A5.37 5.37 0 0 1 24 5.383v13.235A5.37 5.37 0 0 1 18.618 24H5.382A5.37 5.37 0 0 1 0 18.618V5.383A5.37 5.37 0 0 1 5.382.001Z"></path>
              <path fill="#fff" d="M13.269 17.31a1.813 1.813 0 1 0-3.626.002 1.813 1.813 0 0 0 3.626-.002m-.535-6.527H7.213a1.813 1.813 0 1 0 0 3.624h5.521a1.813 1.813 0 1 0 0-3.624m4.417-4.712H8.87a1.813 1.813 0 1 0 0 3.625h8.283a1.813 1.813 0 1 0 0-3.624z"></path>
            </svg>
          </a>
        ...
        >
          <span className="hidden sm:inline">{dictionary.nav.search}</span>
        ...
        <button className="theme-toggle-button p-3 rounded-full border"
                title={dictionary.nav.theme}
                onClick={themeToggleHandler}>
          <img alt={dictionary.nav.theme} height="20px" width="25px"></img>
        </button>
      </div>
    </div>
